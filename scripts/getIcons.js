...
module.exports = function () {
  const solidIcons = getIcons('solid'),
        outlineIcons = getIcons('outline')

  return [...solidIcons, ...outlineIcons]
}
...
function getIcons (dir) {
  const files = fs.readdirSync(`./git_modules/heroicons/${dir}`),
        metadata = files.map(file => ({
          name: `${file.match(fileNameRegexp)[0].replace(fileExtensionRegexp, '')}${dir === 'outline' ? '-outline' : ''}`,
          contents: fs.readFileSync(`./git_modules/heroicons/${dir}/${file}`, 'utf8'),
        }))

  return metadata.map(({ name, contents }) => ({
    componentName: `Heroicons${toComponentName(name)}`,
